...
<body>
    ...
    <script>
        ...
        database.ref('driver').on('child_added', function(data) {
        ...
        database.ref('driver').on('child_changed', function(data) {
            update_data_table(data.val().username, data.val().profile_picture, data.val().email, data.key)
        });
        database.ref('driver').on('child_removed', function(data) {
        ...
        function update_data(name, email, pic, key) {
            database.ref('driver/' + key).update({
                username: name,
                email: email,
                profile_picture: pic
            });
        ...
        $(document).on("click", ".btnEdit", function(event) {
            ...
            database.ref('driver/' + key).once("value").then(function(snapshot) {
        ...
        $(document).on("click", ".btnRemove", function(event) {
            event.preventDefault();
            key = $(this).attr("data-key");
            database.ref('driver/' + key).remove();
        })
